...
abstract class JLoader
{
	...
	public static function import($key, $base = null)
	{
		...
		if (!isset(self::$imported[$key]))
		{
			...
			$base = (!empty($base)) ? $base : JOOMLA_PATH . '/libraries';
			...
			if (strpos($path, 'joomla') === 0)
			{
				...
				} elseif (is_file(JPATH_PLATFORM . '/' . $path . '.php')) {
					
					self::$classes[strtolower($class)] = JPATH_PLATFORM . '/' . $path . '.php';
					$success = true;
				}
			}
			/*
			...
			*/
			else
			{
				...
				if (is_file($base . '/' . $path . '.php'))
				{
					...
					$success = (bool) include_once JPATH_PLATFORM . '/' . $path . '.php';
				}
...
//		self::$prefixes['J'] = array(
//			JPATH_PLATFORM . '/joomla',
//			JOOMLA_PATH . '/libraries/joomla'
//		);
