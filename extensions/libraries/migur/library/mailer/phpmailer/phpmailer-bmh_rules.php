...
function bmhBodyRules($body,$structure,$debug_mode=false) {
  ...
  if (false) {
	 ...
	 */
	elseif (preg_match ("/host not found/is",$body,$match)) {
	 ...
	 * Diagnostic-Code: SMTP; 550 Mailbox ( xxxxx@yourdomain.com ) not found or inactivated
	 */
	elseif (preg_match ("/email[^.]+(?:not found)|(?:not exist)/is",$body,$match)) {
	  $result['rule_cat']    = 'unknown';
	  $result['rule_no']     = '0136';
	}
  ...
  elseif (preg_match ("/delivery[^\n\r]+failed.*\s+(\S+@\S+\w)\s/is",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0013';
    $result['email']       = $match[1];
  }
  ...
  if (empty($result['email'])) {
    ...
    switch ($action) {
      case 'failed':
         ...
         * Diagnostic-Code: SMTP; 550 Mailbox ( xxxxx@yourdomain.com ) not found or inactivated
         */
        elseif (preg_match ("/(?:alias|account|recipient|address|email|mailbox|user|host).*NOT FOUND/is",$diag_code)) {
          $result['rule_cat']    = 'unknown';
          $result['rule_no']     = '0136';
        }
		/* rule: unknown
