...
class NewsletterModelSubscribers extends MigurModelList
{
	...
	public function setDefaultQuery()
	{
		...
		$query->select('a.*');
		$query->from(
			'(SELECT s.subscriber_id, COALESCE(u.name, s.name) AS name, COALESCE(u.email, s.email) AS email, COALESCE(s.state, 1) AS state, u.id AS user_id, COALESCE(s.confirmed, u.activation = "") AS confirmed
